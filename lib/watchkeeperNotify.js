import { getSuperAdmins } from "./owners";
...
export async function deliverWatchkeeperNotification({ settings, ownerEmail, boatName, slug, event }) {
  ...
  if (emailEnabled || superAdmins.length > 0) {
    const extra = emailEnabled && Array.isArray(settings?.watchkeeper_recipients)
      ? settings.watchkeeper_recipients.map(normEmail).filter(Boolean)
      : [];
    const boatRecipients = emailEnabled ? [normEmail(ownerEmail), ...extra] : [];
    const recipients = uniq([...boatRecipients, ...superAdmins].filter(Boolean));
