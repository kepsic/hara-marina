...
export async function quoteStay({ berthId, dockId, arrival, departure, loaM, marinaSlug }) {
  ...
  // Public summary of the LOA tier ladder so the booking wizard can render
  // a "from €X / night, depending on length" hint before the guest enters a
  // boat length. Only the dimensions and price are returned — internal
  // platform-fee config is never leaked here.
  const tiersPublic = (Array.isArray(cfg.loaTiers) ? cfg.loaTiers : [])
    .map((t) => ({
      maxLoaM: t.maxLoaM == null ? null : Number(t.maxLoaM),
      nightCents: Number(t.nightCents),
    }))
    .filter((t) => Number.isFinite(t.nightCents))
    .sort((a, b) => (a.maxLoaM == null ? Infinity : a.maxLoaM) - (b.maxLoaM == null ? Infinity : b.maxLoaM));
  ...
  return {
    ...
    loaTiers: tiersPublic,
    hasLoa: Number.isFinite(Number(loaM)) && Number(loaM) > 0,
  };
}
