...
const BOOKING_ACTION_TTL_DAYS = 14;

/** One-shot link the harbor master clicks from email to approve/reject a booking.
 *  Action is bound into the token so an "approve" link cannot be used to reject. */
export async function signBookingActionToken({ bookingId, action }) {
  return await new SignJWT({ bookingId, action, kind: "booking_action" })
    .setProtectedHeader({ alg: "HS256" })
    .setIssuedAt()
    .setExpirationTime(`${BOOKING_ACTION_TTL_DAYS}d`)
    .sign(getSecret());
}
...
export async function verifyBookingActionToken(token) {
  ...
  try {
    const { payload } = await jwtVerify(token, getSecret());
    if (payload.kind !== "booking_action" || !payload.bookingId || !payload.action) return null;
    return payload;
  } catch {
    return null;
  }
...
export async function sendMagicLink({ email, link }) {
  ...
  const from = process.env.RESEND_FROM || "MerVare <onboarding@resend.dev>";
  if (!apiKey) {
  ...
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: [email],
      subject: "⚓ MerVare sign-in link",
      html: `<p>Click to sign in to MerVare:</p>
