/**
 * Shared Redis client backed by ioredis (Railway Redis).
 *
 * Exposes the methods used across the codebase:
 * get, set, hgetall, hset, expire, lpush, lrange, ltrim
 * with JSON auto-parse/serialise semantics.
 *
 * Configure via env:
...
export class Redis {
   ...
   * When nx=true, returns "OK" if set, null if key already existed (atomic).
   */
  async set(key, value, opts) {
    ...
    if (args.length > 0) {
      return this.client.set(key, str, ...args);
    }
