...
export async function readTelemetryHistory(slug, opts = {}) {
  ...
  // PostgREST has a hard server-side row cap (default 1000) that .limit()
  // alone can't bypass, and even with paging the most-recent N rows over
  // a 7-day / 30-day window only covers the last few hours when the boat
  // streams every ~15 s. So for any window longer than what a single page
  // can comfortably cover, split it into K equal time buckets and pull
  // the most-recent ~limit/K rows from each in parallel. The result is
  // an evenly-spaced sample of the whole window.
  const PAGE = 1000;
  const buckets = Math.max(1, Math.min(20, Math.ceil(windowMs / (6 * 60 * 60 * 1000)))); // 1 bucket per ~6h, capped 1..20
  const perBucket = Math.max(1, Math.min(PAGE, Math.ceil(limit / buckets)));
  const bucketMs = windowMs / buckets;

  const queries = [];
  for (let i = 0; i < buckets; i++) {
    const fromMs = sinceMs + i * bucketMs;
    const toMs = i === buckets - 1 ? nowMs : sinceMs + (i + 1) * bucketMs;
    queries.push(
      sb.from("telemetry_history")
        .select("*")
        .eq("slug", slug)
        .gte("ts", new Date(fromMs).toISOString())
        .lt("ts", new Date(toMs).toISOString())
  ...
  try {
    ...
    console.error("[supabase] history read failed:", e?.message || e);
    return [];
  }
  const rows = [];
  for (const { data, error } of results) {
    if (error) {
      console.error("[supabase] history bucket failed:", error.message);
      continue;
    }
    if (data) rows.push(...data);
  }
  rows.sort((a, b) => new Date(a.ts) - new Date(b.ts));
  // Trim to limit if we overshot.
  if (rows.length > limit) {
    const stride = rows.length / limit;
    const trimmed = [];
    for (let i = 0; i < limit; i++) trimmed.push(rows[Math.floor(i * stride)]);
    return trimmed;
  }
  return rows;
}
