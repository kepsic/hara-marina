...
export async function readTelemetryHistory(slug, opts = {}) {
  ...
  // IMPORTANT: order DESC + limit so we always get the most-recent N rows
  // in the window. With ascending+limit, a busy window (e.g. ~240 rows/hour
  // over 24h ≫ 5000-row cap) returned the oldest N rows and the chart
  // appeared to be stuck ~24h in the past. Reverse to chronological for
  // callers that expect ascending order.
