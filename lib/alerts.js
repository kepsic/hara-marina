...
import { classifyMarinaState } from "./marina";
...
// Decide whether to suppress the offline alert based on last known position.
// A boat that was last seen underway, anchored away from the marina, or in any
// non-moored state is expected to lose connection at sea — do not email.
// Only suppress when classification is confident: missing position returns
// suppress=false so a moored boat that never reported GPS still alerts.
function shouldSuppressOffline(lastKnown) {
  if (!lastKnown || typeof lastKnown !== "object") return { suppress: false, reason: "no-telemetry" };
  const lat = Number(lastKnown?.position?.lat);
  const lon = Number(lastKnown?.position?.lon);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return { suppress: false, reason: "no-position" };
  const sog = Number(lastKnown?.sog_kn);
  const cls = classifyMarinaState({ lat, lon, sog: Number.isFinite(sog) ? sog : 0 });
  if (cls.state === "moored") return { suppress: false, reason: "moored", state: cls.state };
  if (cls.state === "anchored_nearby") return { suppress: false, reason: "anchored-nearby", state: cls.state };
  if (cls.state === "unknown") return { suppress: false, reason: "unknown-state" };
  // underway or away from marina — connection loss at sea is expected.
  return { suppress: true, reason: cls.state, distance_m: cls.distanceM };
}
...
export async function evaluateAndNotifyAlerts(slug, telemetry, options = {}) {
  ...
  // Fetch last known telemetry for context (position, SOG) so we can suppress
  // the offline alert when the boat is underway or away from the marina — a
  // sailing yacht losing cell coverage at sea is normal and not actionable.
  const lastKnownTelemetry = telemetry || (await getTelemetry(cleanSlug).catch(() => null));
  ...
  // Apply context-aware suppression for the offline rule. We force-clear the
  // active state instead of just skipping the email so the alert does not show
  // up as a stuck red banner on the dashboard either.
  const offlineSuppression = shouldSuppressOffline(lastKnownTelemetry);
  if (offlineSuppression.suppress) {
    for (const row of evaluations) {
      if (row.rule !== "offline") continue;
      if (row.next.active || row.prev.active) {
        const wasActive = row.prev.active;
        row.next.active = false;
        row.next.first_triggered_ts = null;
        row.next.pending_trigger = false;
        row.next.pending_since = null;
        row.next.acked_at = null;
        row.next.snoozed_until = null;
        row.status = wasActive ? "resolved" : "unchanged";
        row.ignored = true;
        logAlert(cleanSlug, row.rule, "suppressed (vessel underway/away)", offlineSuppression);
      }
    }
  }
