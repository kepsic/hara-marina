...
function publicBase() {
  const raw = process.env.MARINA_PUBLIC_URL || process.env.NEXT_PUBLIC_SITE_URL || "https://hara.mervare.app";
  return raw.replace(/\/$/, "");
}
...
async function send({ to, cc, bcc, replyTo, subject, html, text }) {
  ...
  const FALLBACK_FROM = "MerVare <onboarding@resend.dev>";
...
const MARINA = {
  ...
  portMapsUrl: process.env.MARINA_PORT_MAPS_URL || "https://harasadam.ee/en/hara-port/#Port-maps",
  portRulesUrl: process.env.MARINA_PORT_RULES_URL || "https://harasadam.ee/en/hara-port/#Port-rules",
...
export async function sendBookingConfirmed(booking) {
  ...
  const html = `
    ...
    <ul>
      ...
      <li>Your berth: <b>${escapeHtml(booking.berthLabel || booking.berthId)}</b> on dock <b>${escapeHtml(booking.dockName || booking.dockId)}</b> — see the <a href="${escapeHtml(MARINA.portMapsUrl)}">port maps</a> for the dock layout and approach.</li>
      ...
      <li>Please skim the <a href="${escapeHtml(MARINA.portRulesUrl)}">port rules</a> before arrival — speed limits, waste, fire safety, etc.</li>
    </ul>
