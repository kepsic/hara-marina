...
// Default marina slug for legacy single-tenant deployments. All key helpers
// fall back to this when no marinaSlug is passed in.
const DEFAULT_SLUG = process.env.DEFAULT_MARINA_SLUG || "hara";
const bookingsKey = (slug) => keys.bookings(slug || DEFAULT_SLUG);
...
async function lookupLabels(dockId, berthId, marinaSlug) {
  try {
    const layout = await redis.get(layoutKey(marinaSlug));
...
export async function listBookings({ marinaSlug, berthId, status, fromIso, toIso } = {}) {
  const raw = await redis.get(bookingsKey(marinaSlug));
...
export async function getBooking(id, marinaSlug) {
  const raw = await redis.get(bookingsKey(marinaSlug));
...
export async function createBooking(input) {
  ...
  const quote = await quoteStay({
    ...
    arrival,
    departure,
    loaM: Number(input.loaM),
    marinaSlug: input.marinaSlug || DEFAULT_SLUG,
  });
  const now = new Date().toISOString();
  const slug = input.marinaSlug || DEFAULT_SLUG;
  const labels = await lookupLabels(input.dockId, input.berthId, slug);
  const booking = {
    id: randomId(),
  ...
  const all = (await redis.get(bookingsKey(slug))) || [];
  all.push(booking);
  await redis.set(bookingsKey(slug), all);
  return booking;
}

export async function updateBooking(id, patch, marinaSlug) {
  const all = (await redis.get(bookingsKey(marinaSlug))) || [];
  ...
  all[idx] = next;
  await redis.set(bookingsKey(marinaSlug), all);
  return next;
}

export async function cancelBooking(id, reason, marinaSlug) {
  return updateBooking(id, { status: "cancelled", cancelledReason: reason || "" }, marinaSlug);
}
