...
EMTRAK_MODE="${EMTRAK_MODE:-auto}"
EMTRAK_ADDRESS="${EMTRAK_ADDRESS:-192.168.1.1:39150}"
EMTRAK_SERIAL_DEVICE="${EMTRAK_SERIAL_DEVICE:-}"
EMTRAK_SERIAL_BAUD="${EMTRAK_SERIAL_BAUD:-38400}"
...
umask 077
cat > "$ENV_FILE" <<EOF
# Transport: auto (default) probes USB serial /dev/ttyACM* first, then
# falls back to TCP. Override with EMTRAK_MODE=tcp or EMTRAK_MODE=serial.
EMTRAK_MODE=$EMTRAK_MODE
EMTRAK_ADDRESS=$EMTRAK_ADDRESS
EMTRAK_SERIAL_DEVICE=$EMTRAK_SERIAL_DEVICE
EMTRAK_SERIAL_BAUD=$EMTRAK_SERIAL_BAUD
...
exec 2>&1
. $ENV_FILE
export EMTRAK_MODE EMTRAK_ADDRESS EMTRAK_SERIAL_DEVICE EMTRAK_SERIAL_BAUD
export AIS_INGEST_URL AIS_INGEST_TOKEN AIS_INGEST_MMSI AIS_INGEST_NAME
